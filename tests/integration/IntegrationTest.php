...
namespace ArnaudRitti\Composer\Installers\GravityForms\Test\Integration;
...
class IntegrationTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $process = new Process(
            [
                ...
                "build",
                "-t",
                "gravityforms-installer/registry:latest",
                "."
            ],
        ...
        $process = new Process(["docker", "ps", "-f", "name=gravityforms-installer-registry"]);
        $process->mustRun();
        if (strstr($process->getOutput(), 'gravityforms-installer-registry') !== false) {
            $process = new Process(["docker", "rm", "-f", "gravityforms-installer-registry"]);
            $process->mustRun();
        }

        // If the network exists, remove it first
        $process = new Process(["docker", "network", "ls", "-f", "name=gravityforms-installer-test"]);
        $process->mustRun();
        if (strstr($process->getOutput(), 'gravityforms-installer-test') !== false) {
            $process = new Process(["docker", "network", "rm", "gravityforms-installer-test"]);
            $process->mustRun();
        }

        $process = new Process(["docker", "network", "create", "--driver", "bridge", "gravityforms-installer-test"]);
        $process->mustRun();
        ...
        $process = new Process(
            [
                ...
                "-d",
                "-i",
                "--network=gravityforms-installer-test",
                "--network-alias=gravityapi.com",
                "--name",
                "gravityforms-installer-registry",
                "--rm",
                "gravityforms-installer/registry:latest"
            ]
        );
            ...
            foreach (self::$composerVersions as $shortComposerVersion => $fullComposerVersion) {
                ...
                $process = new Process(
                    [
                        ...
                        "-t",
                        "gravityforms-installer/testapp:{$shortPhpVersion}-{$shortComposerVersion}",
                        "."
                    ],
                    __DIR__ . "/images/app"
                );
    ...
    public function tearDown(): void
    {
        ...
        $registry = new Process(
            ["docker", "logs", "gravityforms-installer-registry", "--since", $this->testStartedAt->getTimestamp()]
        );
    ...
    public function testWithSpecificVersionInstallWorksCorrectly($phpVersion, $composerVersion)
    {
        ...
        $process = new Process(
            [
                ...
                "--network=gravityforms-installer-test",
                "-e",
                "GRAVITYFORMS_KEY=938C927AFC694954A84476CF3CBD28B3",
                "-v",
                "{$localComposerPath}:/app/composer.json",
                "gravityforms-installer/testapp:{$phpVersion}-{$composerVersion}"
            ],
            __DIR__
        );
    ...
    public function testWithComposerConfigKeyWorksCorrectly($phpVersion, $composerVersion)
    {
        ...
        $configContents = '{\\"config\\":{\\"gravityforms-key\\":\\"938C927AFC694954A84476CF3CBD28B3\\"}}';
    ...
    public function testWithBedrockInstallWorksCorrectly($phpVersion, $composerVersion)
    {
        ...
        $composerData->require->{"arnaud-ritti/gravityforms-installer"} = "dev-master";
        $composerData->require->{"gravityforms/gravityforms"} = "dev-master";
